...
QWidget* LoginRegisterDlg::CreateLoginWidget()
{
	...
	m_autoLoginChx->setChecked(config->value("user/auto_login").toBool());
	m_rememberPwdChx->setChecked(config->value("user/remember_pwd").toBool());
	connect(m_autoLoginChx, &QCheckBox::stateChanged, [=](int checked)
		{
			...
			config->setValue("user/auto_login", checked);
		});
	...
	connect(m_rememberPwdChx, &QCheckBox::stateChanged, [=](int checked)
		{
			...
			config->setValue("user/remember_pwd", checked);
		});
	...
	if (config->value("user/auto_login").toBool())loginBtn->click();
		...
		.success([=](const QByteArray& data)
			{
				...
				if (jdom["code"].toInteger() < 1000)
				{
					...
					config->setValue("user/rember_pwd", isRememberPwd);
					config->setValue("user/auto_login", m_autoLoginChx->isChecked());
