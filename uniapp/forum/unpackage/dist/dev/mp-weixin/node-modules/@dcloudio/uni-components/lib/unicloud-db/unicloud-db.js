...
const _sfc_main = {
  name: "UniClouddb",
  setup(props) {
    const dataListRef = props.ssrKey ? props.getone ? common_vendor.shallowSsrRef(void 0, props.ssrKey) : common_vendor.ssrRef([], props.ssrKey) : props.getone ? common_vendor.shallowSsrRef(void 0, "Bu7PARd7N+D+EACeQqWQrA==") : common_vendor.ssrRef([], "FKB/WBK3EMqW2Pb/FX47xw==");
  ...
  methods: {
    ...
    add(value, {
      ...
      let db = common_vendor.Zs.database(this.spaceInfo);
      if (action) {
        db = db.action(action);
      }
    ...
    _execRemove(id, action, success, fail, complete, needConfirm, needLoading, loadingTitle) {
      ...
      const db = common_vendor.Zs.database(this.spaceInfo);
